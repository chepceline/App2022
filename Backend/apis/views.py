...
import json
from .models import CustomUser,Scenario,ScenarioUser,ScenarioSolution
...
def get_final_score_unsupervised(model, train_data, test_data, outliers_data, config_weights, mappings_config, factsheet, solution_set_path, recalc=False):
    ...
    for pillar in scores.items():
        ...
        if pillar[0] == 'fairness':
            # print("Pillar fairness is:", pillar[1])
            fairness_score = int(pillar[1]['underfitting'])*0.35 +  int(pillar[1]['overfitting'])*0.15 
            + int(pillar[1]['statistical_parity_difference'])*0.15 + int(pillar[1]['disparate_impact'])*0.1 
        ...
        if pillar[0] == 'explainability':
            ...
            if str(pillar[1]['permutation_feature_importance']) != 'nan':
                feature_relevance= int(pillar[1]['permutation_feature_importance'])*0.15
...
# path_mapping_accountabiltiy=os.path.join(BASE_DIR,'apis/MappingsWeightsMetrics/Mappings/Accountability/default.json')
# path_mapping_fairness=os.path.join(BASE_DIR,'apis/MappingsWeightsMetrics/Mappings/explainability/default.json')
print("Final Score unsupervised:", get_final_score_unsupervised(path_module, path_traindata, path_testdata, outliers_data, config_weights, mappings_config, path_factsheet, solution_set_path))
        ...
        def get_final_score_unsupervised(model, train_data, test_data, outliers_data, config_weights, mappings_config, factsheet, solution_set_path, recalc=False):
            ...
            for pillar in scores.items():
                ...
                if pillar[0] == 'fairness':
                    ...
                    uploaddic['unsupervised_underfitting'] = int(pillar[1]['underfitting'])
                    uploaddic['unsupervised_overfitting'] = int(pillar[1]['overfitting'])
                    uploaddic['unsupervised_statistical_parity_difference'] = int(pillar[1]['statistical_parity_difference'])
                    uploaddic['unsupervised_disparate_impact'] = int(pillar[1]['disparate_impact'])
...
class solution(APIView):
    ...
    def post(self, request):
        if request.data is not None:
            ...
            # print("Solution type:", request.data['Solutiontype'])
            fileupload = ScenarioSolution.objects.create(
                ...
                TestFile=request.data['TestFile'],
